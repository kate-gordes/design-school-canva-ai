...
export default function Stickers(): React.ReactNode {
  const { brandKitData } = useAppContext();

  // Get Stickers category from the selected brand kit
  ...
  const cleanFolders: FolderData[] = useMemo(() => {
    ...
    stickersCategory.folders.forEach((folder: Folder) => {
  ...
  const stickersDesigns: (ProjectDesign & { imageUrl?: string })[] = useMemo(() => {
    if (!stickersCategory || !stickersCategory.assets) return [];

    return stickersCategory.assets.map((asset: BrandKitAsset) => ({
      id: asset.id,
  ...
  const assetCount = stickersCategory?.assetCount || 0;
    ...
    <Rows spacing="3u">
      ...
      <Box paddingTop="4u">
        <BrandHeader
          ...
          showAddNew={false}
          showMore={false}
          onAddNewClick={() => console.log('Add new sticker')}
        ...
        <Box width="full">
          {stickersDesigns.length > 0 ? (
            <DesignSection
              title={`Assets (${stickersDesigns.length})`}
            ...
            <Box padding="6u">
              <Text tone="secondary">No stickers available ({assetCount} assets)</Text>
            </Box>
          )}
        </Box>
      </Box>
